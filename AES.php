...
class Aes
{
    ...
    */
    public function __construct($z, $mode = 'ECB', $iv=null) //$iv='ABCDEFGHIJ123456', jika pake CBC)
    {
        $this->mode = strtoupper($mode);
        $this->iv = $iv;
        //$this->iv = str_shuffle($iv); Jika pake CBC
        $this->Nk = strlen($z)/4;
        ...
        if ($this->Nk != 4 ) {
            die('Key is ' . ($this->Nk*32) . ' bits long. *not* 128.');
        }
